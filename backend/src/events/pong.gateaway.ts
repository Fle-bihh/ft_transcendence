...
export class PongGateway implements OnGatewayConnection, OnGatewayDisconnect {
  ...
  getRoomByID(roomID: string): [number, GameClass] | null {
    for (let i = 0; i < this.allGames.length; i++)
      if (this.allGames[i].roomID == roomID) return [i, this.allGames[i]];
    return null;
  }
  ...
  getRoomByClientLogin(ClientLogin: string): [number, GameClass] | null {
    for (let i = 0; i < this.allGames.length; i++) {
      for (let j = 0; j < 2; j++) {
        if (
          this.allGames[i].players[j] != null &&
          this.allGames[i].players[j].username == ClientLogin
        ) {
  ...
  async render(roomID: string) {
    ...
    if (room != null) {
      if (
        this.allGames[room[0]].players[0].score == 3 ||
        this.allGames[room[0]].players[1].score == 3
      ) {
        const data = {
          ...
          winner_id:
            room[1].players[0].score === 3
              ? room[1].players[0].username
              : room[1].players[1].username,
        ...
        this.io
          .to(room[1].roomID)
          .emit('finish', { room: room[1], draw: false });
        return;
      }
  ...
  finishGameDeco(data: {
    ...
    if (room != null) {
      ...
      if (
        ...
        this.io
          .to(room[1].roomID)
          .emit('finish', { room: room[1], draw: true });
      }
    }
  }
  ...
  checkDeco() {
    for (let index = 0; index < this.allGames.length; index++) {
      ...
      if (this.allGames[index].gameOn) {
        ...
        console.log(
          'ready 0 : ',
          this.allGames[index].players[0].ready,
          ' ready 1 : ',
          this.allGames[index].players[1].ready,
        );
        console.log(
          'reco 0 : ',
          this.allGames[index].players[0].reco,
          ' reco 1 : ',
          this.allGames[index].players[1].reco,
        );
        if (
          !this.allGames[index].players[0].ready &&
          this.allGames[index].players[1].ready &&
        ...
        if (millis0 != 0 && 10 - Math.floor(millis0 / 1000) === 0)
          this.finishGameDeco({
            roomId: this.allGames[index].roomID,
            playerwin: 1,
            playerdeco: 0,
          });
  ...
  handleDisconnect(client: Socket) {
    ...
    console.log('client deco pong gateway')
    ...
    allClients.splice(
      allClients.findIndex((item) => item.id == client.id),
      1,
    );
  }
  ...
  async checkReconnexion(client: Socket, user: { username: string }) {
    ...
    if (room != null) {
      ...
      this.allGames[room[0]].players[
        this.allGames[room[0]].players.findIndex(
      ...
      allClients.forEach((client) => {
        this.io.to(client.id).emit('getClientStatus', {
          user: this.allGames[room[0]].players[
            this.allGames[room[0]].players.findIndex(
              (player) => player.username == user.username,
            )
          ].username,
          status: 'in-game',
          emitFrom: 'CHECK_RECONNEXION',
        });
      });
  ...
  clientStatusGame() {
    UserDisconnected.forEach(async (user, index) => {
      if (new Date().getSeconds() - user.date.getSeconds() != 0) {
        ...
        this.logger.log(
          `[Pong-Gateway] Client \'${user.username}\' disconnect`,
        );
        waitingForGame.splice(
          waitingForGame.findIndex((item) => item.user.login == user.username),
          1,
        );
  ...
  async handleInterval() {
    for (let index = 0; index < this.allGames.length; index++) {
      if (this.allGames[index].gameOn) {
        if (
          !(
            this.allGames[index].players[0].score == 3 ||
            this.allGames[index].players[1].score == 3
          )
        )
          if (
            this.allGames[index].players[0].ready &&
    ...
    if (room != null) {
      for (let index = 0; index < 2; index++)
        if (this.allGames[room[0]].players[index].id == client.id) {
          ...
          if (info[1]) this.allGames[room[0]].players[index].goDown = false;
        }
    }
  }
    ...
    if (room != null) {
      for (let index = 0; index < 2; index++)
        if (this.allGames[room[0]].players[index].id == client.id) {
          ...
          if (info[1]) this.allGames[room[0]].players[index].goUp = false;
        }
    }
  }
  ...
  async startGame(
    client: Socket,
    info: { user: { login: string }; gameMap: string },
  ) {
    let oponnent: {
      map: string;
      user: {
    ...
    if (oldGame != null)
      this.finishGameDeco({
        roomId: oldGame[1].roomID,
        playerwin: oldGame[1].players[0].username == info.user.login ? 1 : 0,
        playerdeco: oldGame[1].players[0].username == info.user.login ? 0 : 1,
      });
    if (
      waitingForGame.findIndex((item) => item.user.login == info.user.login) !=
      -1
    )
      waitingForGame.splice(
        waitingForGame.findIndex((item) => item.user.login == info.user.login),
    ...
    if (
      ...
      this.allGames.push(
        new GameClass(
          info.gameMap,
          info.user.login,
          info.user.login + oponnent.user.login,
          client.id,
        ),
      );
      const room = this.getRoomByID(info.user.login + oponnent.user.login);
      this.allGames[room[0]].players[0].connect = true;
      this.allGames[room[0]].setOponnent(
        allClients.find((client) => client.username == oponnent.user.login).id,
        oponnent.user.login,
      );
      ...
      allClients
        .find((client) => client.username == oponnent.user.login)
        .socket.emit('joinRoom', room[1].roomID);
      allClients
        .find((client) => client.username == oponnent.user.login)
        .socket.emit('start', room[1].roomID);
      this.io.to(client.id).emit('start', room[1].roomID);
      waitingForGame.splice(
        waitingForGame.findIndex(
          (item) => item.user.login == oponnent.user.login,
        ),
        1,
      );
    ...
    if (this.allGames.length != 0) {
      this.allGames.map((room) => {
        if (
          ...
          room.players[1].username != username
        ) {
          game = 'yes';
  ...
  start_spectate(client: Socket, data: { roomID: string; start: boolean }) {
    ...
    if (room) {
      if (data.start == false) this.joinRoom(client, room[1].roomID);
  ...
  inviteGame(
    ...
    if (receiver) {
      ...
      if (!room) this.io.to(receiver.id).emit('invite_game', data);
    } else this.io.to(client.id).emit('cant_invite', data);
  }
  ...
  declineGame(
    ...
    if (sender) this.io.to(sender.id).emit('decline_game', data);
  }
  ...
  async startInviteGame(
    client: Socket,
    info: { user: { login: string }; gameMap: string; roomId: string },
  ) {
    let oponnent: { map: string; user: { login: string } };
    if (
      (oponnent = waitingForInvite.find((item) => item.map == info.gameMap)) !=
      undefined
    ) {
      this.allGames.push(
        new GameClass(info.gameMap, info.user.login, info.roomId, client.id),
      );
      ...
      allClients
        ...
        .socket.emit('start', room[1].roomID);
      this.io.to(client.id).emit('start', room[1].roomID);
      waitingForInvite.splice(
        waitingForInvite.findIndex((item) => item.map == info.gameMap),
        1,
      );
  ...
  getUserStatus(client: Socket, data: { nickname: string }) {
    ...
    if (allClients.find((item) => item.username == data.nickname))
      this.io.to(client.id).emit('getClientStatus', {
        user: data.nickname,
        status: 'online',
        emitFrom: 'clientStatusGame',
